...
const ERRORS = {
  ...
  EMPTY_ENTRE: "'entre' no puede estar vac\u00edo",
  UNKNOWN_ENTRE: "Participante en 'entre' no reconocido",
  DUPLICATE_ENTRE: "Participante duplicado en 'entre'",
};
...
function validateEntre(entre, participants) {
  if (entre === undefined) return undefined;
  if (!Array.isArray(entre) || entre.length === 0) {
    throw new Error(ERRORS.EMPTY_ENTRE);
  }
  const cleaned = entre.map(validateName);
  const seen = new Set();
  for (const p of cleaned) {
    if (!participants.includes(p)) {
      throw new Error(ERRORS.UNKNOWN_ENTRE);
    }
    if (seen.has(p)) {
      throw new Error(ERRORS.DUPLICATE_ENTRE);
    }
    seen.add(p);
  }
  return cleaned;
...
function agregarGasto(grupo, gasto) {
  ...
  const { concepto, importe, pagadoPor, entre } = gasto;
  ...
  const cleanedEntre = validateEntre(entre, grupo.participantes);
  const newGasto = {
  ...
  if (cleanedEntre !== undefined) {
    newGasto.entre = cleanedEntre;
  }
...
function balances(grupo) {
  ...
  for (const g of grupo.gastos) {
    const reparto = g.entre || grupo.participantes;
    const cuota = g.importe / reparto.length;
    for (const p of reparto) {
      result[p] -= cuota;
    }
    result[g.pagadoPor] += g.importe;
  }
  for (const p of grupo.participantes) {
    result[p] = ROUNDED(result[p]);
  }
  return result;
}

function liquidacion(grupo) {
  // Compute exact balances (same entre-aware logic as balances)
  const exactBalances = {};
  for (const p of grupo.participantes) {
    exactBalances[p] = 0;
  }
  for (const g of grupo.gastos) {
    ...
    const cuota = g.importe / reparto.length;
    for (const p of reparto) {
      exactBalances[p] -= cuota;
    }
    exactBalances[g.pagadoPor] += g.importe;
  }
  const deudores = [];
  const acreedores = [];
